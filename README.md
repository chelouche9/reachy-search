...
> Ask "What is this?" and it says: *"That's a stovetop moka pot. Water boils in the bottom
...
1. **Say "hey Reachy".** A local Whisper model listens for its name. Audio
   ...
   pleasantries. Booping an antenna works too, since the antennas are semi-passive,
   so they double as physical buttons.
2. **Ask out loud.** It grabs a camera frame as you finish talking and records
   until you stop.
3. **It thinks out loud.** Claude identifies the object and writes a search
...
They're defined as functions of time in `reachy_search/moves.py`, with no recorded
...
| `brain.py` | The agent: frame and question in, decides whether to search, answer out |
| `search.py` | Tavily |
...
`rs.reset()` clears it. Runs anywhere. `mini` is optional (used only to grab a
...
r.results        # [{title, url, snippet}, ...] plus r.images when relevant
